...
Deno.test("formatDate function - common patterns", () => {
  ...
  assertEquals(formatDate(date, 'YYYY-MM-DD [at] HH:mm'), '2024-12-25 at 14:35');
});
...
Deno.test("formatDate function - literals with brackets", () => {
  ...
  assertEquals(formatDate(date, '[Today is] YYYY-MM-DD'), 'Today is 2024-06-15');
  assertEquals(formatDate(date, 'YYYY-MM-DD [at] HH:mm:ss'), '2024-06-15 at 14:35:22');
  assertEquals(formatDate(date, '[[[escaped brackets]]]'), '[[escaped brackets]]');
});
...
Deno.test("formatDate function - UTC timezone", () => {
  ...
  assertEquals(formatDate(date, 'YYYY-MM-DD HH:mm:ss', { tz: 'UTC' }), '2024-06-15 14:35:22');
});
