import { chronal, datesUntil } from "../src/mod.ts";
...
const daily = datesUntil(start1, end1);
...
const weekly = datesUntil(start2, end2, { weeks: 1 });
...
const monthly = datesUntil(start3, end3, { months: 1 });
...
const every3days = datesUntil(start4, end4, { days: 3 });
every3days.forEach((date) =>
...
const chainable = start5.until(end5, { months: 1 });
...
const hourly = datesUntil(start7, end7, { hours: 2 });
...
const yearly = datesUntil(start8, end8, { years: 1 });
